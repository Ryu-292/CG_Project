...
fn fs_main(@builtin(position) fragCoord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    return vec4<f32>(uv, 0.5, 1.0);
      ...
      // 1. The source of truth for the sphere
        const sphereState = {
            x: 0.0,
            y: 0.0,
            z: 0.0,
            r: 1.0,
            color: [1.0, 0.3, 0.3] // RGB
        };
        //torus state
        const torusState = {
            x: 0.0,
        ...
        const orbit = {
            ...
            phi: 1.57,       // Vertical angle (starts roughly at 90deg)
            target: [0, 0, 0],
            isDragging: false,
            lastMouse: { x: 0, y: 0 }
        };

        // 2. Helper to convert Hex color to 0-1 Floats
        ...
        // 3. Update state when sliders move
        document.getElementById('sphere-x').oninput = e => sphereState.x = parseFloat(e.target.value);
        document.getElementById('sphere-y').oninput = e => sphereState.y = parseFloat(e.target.value);
        document.getElementById('sphere-z').oninput = e => sphereState.z = parseFloat(e.target.value);
        document.getElementById('sphere-radius').oninput = (e) => {
            const value = parseFloat(e.target.value);
            
            // 1. Update the GPU state (The Sphere)
            sphereState.r = value;
            
            // 2. Update the HTML Text (The Number)
            document.getElementById('radius-val').textContent = value.toFixed(2);
        };
        document.getElementById('sphere-color').oninput = e => sphereState.color = hexToRgb(e.target.value);

        //torus editor
        document.getElementById('torus-x').oninput = e => torusState.x = parseFloat(e.target.value);
        document.getElementById('torus-y').oninput = e => torusState.y = parseFloat(e.target.value);
        document.getElementById('torus-z').oninput = e => torusState.z = parseFloat(e.target.value);
        document.getElementById('torus-radius').oninput = (e) => {
            const value = parseFloat(e.target.value);
        ...
        document.getElementById('torus-thickness').oninput = (e) => {
            const value = parseFloat(e.target.value);
            
           
            torusState.thickness = value;
            
            document.getElementById('thickness-val').textContent = value.toFixed(2);
        };
        document.getElementById('torus-color').oninput = e => torusState.color = hexToRgb(e.target.value);
...
@group(0) @binding(0) var<uniform> uniforms: Uniforms;`;
      ...
      async function initWebGPU() {
        ...
        sceneBuffer = device.createBuffer({
          size: 80, // Sphere(32) + Torus(48) = 80 bytes
          usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
        });
      ...
      function render() {
        ...
        const dynamicSceneData = new Float32Array([
            sphereState.x, sphereState.y, sphereState.z, sphereState.r, // pos + radius
            sphereState.color[0], sphereState.color[1], sphereState.color[2], 0.0, // color + padding

            torusState.x, torusState.y, torusState.z, torusState.r, // pos + radius
            torusState.thickness, 0.0, 0.0, 0.0, // params + padding (3 floats)
            torusState.color[0], torusState.color[1], torusState.color[2], 0.0 // color + padding
        ]);
        device.queue.writeBuffer(sceneBuffer, 0, dynamicSceneData);
